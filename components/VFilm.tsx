...
const VFilm: React.FC<VFilmProps> = ({ user, onOpen, focusSeriesId, onFocusConsumed, onOpenAgenda, onOpenSeriesElsewhere }) => {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const recomendadas = await api.getRecommendations('vcine');
      ...
      try {
        ...
        setSeries(todas.filter(s => s.content_type === 'vcine'));
      } catch {
  ...
  return (
    <div className="h-full w-full bg-[var(--bg-color)] overflow-y-auto pb-40 scrollbar-hide">
      <header className="p-8 pt-16 md:p-12 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black premium-text tracking-tighter mb-2">VERTICALSHOW</h1>
          <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] ml-1">{t('feed.vcineTagline')}</p>
        </div>
        <button
