...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onAddEpisode, onAddComic, onAddLesson }) => {
  ...
  const handleEpisodeSubmit = (e: React.FormEvent) => {
    ...
    alert("Vídeo publicado no HQCINE!");
  };
  ...
  const handleComicSubmit = (e: React.FormEvent) => {
    ...
    onAddComic({
      ...
      title: coTitle,
      author: coAuthor,
      description: 'Publicação oficial do autor via Painel Admin.',
  ...
  const handleLessonSubmit = (e: React.FormEvent) => {
    ...
    setLeTitle(''); setLeUrl('');
    alert("Aula publicada na ACADEMIA!");
  };

  return (
    <div className="h-full w-full bg-[#0A0A0B] text-white p-8 md:p-20 overflow-y-auto animate-apple font-lailai">
      <header className="mb-12">
        <h1 className="text-4xl md:text-6xl font-black tracking-tighter mb-4">Painel do Proprietário</h1>
        <p className="text-zinc-500 font-medium">Controle total sobre HQCINE, HI-QUA e AULAS da Academia.</p>
      </header>
      ...
      <div className="max-w-xl bg-[#1C1C1E] rounded-[2.5rem] p-10 border border-white/5 shadow-2xl">
        {activeTab === 'hqcines' && (
          <form onSubmit={handleEpisodeSubmit} className="space-y-6">
            ...
            <AdminInput label="Título do Curta" value={epTitle} onChange={setEpTitle} placeholder="Título impactante" />
            <AdminInput label="Sinopse Curta" value={epDesc} onChange={setEpDesc} placeholder="Descreva o vídeo..." />
            <AdminInput label="URL do Vídeo (MP4)" value={epUrl} onChange={setEpUrl} placeholder="https://..." />
            <button type="submit" className="w-full bg-rose-600 py-5 rounded-2xl font-black hover:bg-rose-500 transition-all">Upload para HQCINE</button>
          </form>
        )}
        ...
        {activeTab === 'hiqua' && (
          <form onSubmit={handleComicSubmit} className="space-y-6">
            ...
            <AdminInput label="Nome do Autor" value={coAuthor} onChange={setCoAuthor} placeholder="Créditos do artista" />
            <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-600 uppercase ml-2 tracking-widest">Painéis (URLs separadas por vírgula)</label>
                <textarea 
            ...
            <button type="submit" className="w-full bg-indigo-600 py-5 rounded-2xl font-black hover:bg-indigo-500 transition-all">Upload para HI-QUA</button>
          </form>
        )}
        ...
        {activeTab === 'academia' && (
          <form onSubmit={handleLessonSubmit} className="space-y-6">
            ...
            <AdminInput label="Título da Masterclass" value={leTitle} onChange={setLeTitle} placeholder="Assunto da aula" />
            <AdminInput label="URL do Vídeo (MP4)" value={leUrl} onChange={setLeUrl} placeholder="https://..." />
            <button type="submit" className="w-full bg-emerald-600 py-5 rounded-2xl font-black hover:bg-emerald-500 transition-all text-white">Upload para AULAS</button>
          </form>
        )}
      </div>
    </div>
