...
| `sinalizacaoLimiter.js` (Fase 5, Bloco 3 — Curadoria) | Teto **por USUÁRIO** em `POST /api/content/series/:id/sinalizar`: 30 sinalizações por hora (`sinalizacaoLimiter.js:9-16`). A chave é `req.user.id` (`keyGenerator`, `:14`), **nunca o IP** — por isso o middleware fica DEPOIS de `verifyToken` na rota (`routes/sinalizacao.js:41`), onde `req.user` sempre existe. Existe porque as sinalizações VÁLIDAS já são limitadas pelo unique + consumo real, mas as INVÁLIDAS (`sem_consumo`) eram gravadas sem teto: uma conta podia escrever uma sinalização em cada obra do catálogo em minutos (achado do painel adversarial). Em `NODE_ENV=test` vira passthrough `(req,res,next)=>next()` (`:9-10`), mesmo padrão de `accountLimiter.js:8-9` e do `loginLimiter` (`server.js:184-185`) |
