...
| `notificationService.js` | Fase 4, Bloco 2 — lógica de notificações push. `notifyEpisodePublished(episodeId)` — busca inscritos da série, constrói payload com deep link, enfileira disparo. `getVapidPublicKey()` — exporta a chave pública VAPID (usada pela UI para se inscrever). `__setTransportForTests(transport)` — hook de testes para mock do envio (web-push real fica fire-and-forget) |
