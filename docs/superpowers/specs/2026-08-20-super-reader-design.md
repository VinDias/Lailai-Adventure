...
| Mínimo | `Setting` chave `superReaderMinCents` (default **500**), exposto por rota própria `GET /api/superreader/min` (decisão da T2 — `PUBLIC_KEYS` entregaria a string crua do Setting, sem a validação/fallback do serviço) | O PDF pede mínimo configurável; o frontend lê o mínimo sem auth para montar os botões de valor |
