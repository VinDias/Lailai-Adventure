...
| Proporcionalidade (Etapa 2) | Toda métrica de Qualidade é **por leitor único** (ex.: favoritos ÷ leitores), normalizada contra o melhor do catálogo do mesmo `content_type` | Regra de ouro do PDF: "100 leitores e 20 favoritos > 10.000 leitores e 200 favoritos". Normalizar por tipo evita webtoon competir com vídeo |
...
| Diversidade (Etapa 8) | Re-ordenação da lista final: nenhuma obra 2× (dedupe), não permitir 2 obras ADJACENTES do mesmo `channelId`, intercalar cotas (consolidada/potencial/nova) em vez de blocos | As 4 regras do PDF viram regras mecânicas de lista |
