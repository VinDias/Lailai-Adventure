...
- Suite backend: `npx vitest run tests/backend/<arquivo>` durante a task; suite completa antes do commit final da task. Suite frontend SEMPRE via PowerShell: `npx vitest run tests/frontend/<arquivo>`; `npx tsc --noEmit` antes de commitar frontend. "Worker exited unexpectedly" na suite completa = infra de RAM — critério: 1 passe limpo OU vítimas re-rodadas isoladas verdes.
